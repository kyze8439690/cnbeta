...
import me.yugy.cnbeta.ui.RecommendFragment;
 ...
 */
public class RecommendNewsAdapter extends CursorAdapter2<RecommendNewsAdapter.BaseViewHolder> {

    private RecommendFragment mFragment;

    public RecommendNewsAdapter(RecommendFragment fragment) {
        super(fragment.getActivity(), null, true);
        mFragment = fragment;
    }

    @Override
    public void bindViewHolder(RecommendNewsAdapter.BaseViewHolder holder, Cursor cursor) {
        final News news = News.fromCursor(cursor);
        holder.parse(news);
    }

    @Override
    public RecommendNewsAdapter.BaseViewHolder newViewHolder(ViewGroup viewGroup, int viewType) {
        View view;
        switch (viewType){
